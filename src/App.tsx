...
import { Property, Agent, Inquiry, SavedSearch, ReportedListing, UserProfile } from './types';
import { 
  INITIAL_AGENTS, 
  INITIAL_PROPERTIES, 
  INITIAL_INQUIRIES, 
...
// Icons
import { 
  Building2, Search, Heart, Map, List, SlidersHorizontal, Scale, 
  Sparkles, ShieldCheck, Mail, Compass, HelpCircle, 
  PhoneCall, Star, Bookmark, ClipboardCheck, ArrowUpRight, 
  AlertCircle, ChevronRight, User, AlertOctagon, RotateCcw,
  X, MapPin
} from 'lucide-react';
...
export default function App() {
  ...
  const [reportedListings, setReportedListings] = useState<ReportedListing[]>(INITIAL_REPORTED_LISTINGS);
  ...
  const [currentRole, setCurrentRole] = useState<'seeker' | 'owner' | 'agent' | 'admin'>('seeker');

  // Secure locked Staff portal password terminal challenge variables (AD-02)
  const [isAdminLockScreenVisible, setIsAdminLockScreenVisible] = useState<boolean>(false);
  ...
  const [supportTickets, setSupportTickets] = useState<Array<{
    ...
    userName: string;
    userRole: string;
    messages: Array<{ sender: 'user' | 'admin' | 'bot'; text: string; timestamp: string }>;
  }>>([
    {
      ...
      userRole: 'seeker',
      messages: [
        { sender: 'user', text: 'Hello, I suspect that some properties in the Marina Heights region have duplicate listings. Could you review them?', timestamp: new Date(Date.now() - 3600000).toISOString() },
        { sender: 'bot', text: 'Hello! I am PropFind Guard, your automatic Trust & Safety bot. Your ticket has been logged and assigned to active administrators.', timestamp: new Date(Date.now() - 3590000).toISOString() }
      ]
    },
    {
      ...
      userRole: 'owner',
      messages: [
        { sender: 'user', text: 'Hello administrative team, can you please verify my landlord deed so that I can display the Verified Owner badge on my penthouses?', timestamp: new Date(Date.now() - 7200000).toISOString() },
        { sender: 'bot', text: 'Hello! Your request has been queued for credentials audit.', timestamp: new Date(Date.now() - 7190000).toISOString() }
      ]
    }
  ]);
  ...
  const [lassoFilteredIds, setLassoFilteredIds] = useState<string[] | null>(null);
  ...
  const [activePromotedPropertyId, setActivePromotedPropertyId] = useState<string | null>(null);
  const [promotionDuration, setPromotionDuration] = useState<number>(7);
  const [selectedPromotionType, setSelectedPromotionType] = useState<'featured' | 'spotlight' | 'premium'>('featured');
  const [showPromoteFeedback, setShowPromoteFeedback] = useState<string | null>(null);
  ...
  const [priceDropAlerts, setPriceDropAlerts] = useState<{ id: string; oldPrice: number; newPrice: number; title: string }[]>([]);

  // Local static list of suggested cities for search autocomplete (SR-01)
  const SUGGESTED_CITIES = ['Marina Heights', 'Downtown Core', 'Pine Crest', 'Canyon View', 'Industrial East'];

  // Handle autocomplete matching
  const filteredSuggestions = SUGGESTED_CITIES.filter(city =>
  ...
  const handleToggleFavorite = (propertyId: string) => {
    ...
    if (isSaved) {
      ...
      setProperties(prev => prev.map(p => p.id === propertyId ? { ...p, saves: Math.max(0, p.saves - 1) } : p));
    } else {
      updatedSaves.push(propertyId);
      // Increase save count of property
      setProperties(prev => prev.map(p => p.id === propertyId ? { ...p, saves: p.saves + 1 } : p));
    }
  ...
  const handleToggleCompare = (propertyId: string) => {
    ...
    if (isInList) {
      setComparisonPropertyIds(prev => prev.filter(id => id !== propertyId));
    } else {
      if (comparisonPropertyIds.length >= 4) {
        alert('You can compare up to 4 properties side-by-side inside the comparison matrix.');
        return;
      }
  ...
  const handleSendInquiry = (inqData: Omit<Inquiry, 'id' | 'createdDate'>) => {
    ...
    setProperties(prev => prev.map(p => p.id === inqData.propertyId ? { ...p, inquiryCount: p.inquiryCount + 1 } : p));
  };

  // Dispatch live authentication and complete deferred tasks (US-08)
  const handleAuthenticateUser = (userData: { name: string; email: string; age: number; role: 'seeker' | 'owner' | 'agent' | 'admin' }) => {
    ...
    if (authIntendedAction) {
      if (authIntendedAction.startsWith('favorite:')) {
        ...
        setProperties(prev => prev.map(p => p.id === pId ? { ...p, saves: p.saves + 1 } : p));
        setUserProfile(prev => ({
          ...prev,
        ...
        try {
          ...
          setProperties(prev => prev.map(p => p.id === parsed.propertyId ? { ...p, inquiryCount: p.inquiryCount + 1 } : p));
          alert(`Welcome, passport verified! Your tour arrangement was dispatched.`);
        } catch (err) {
  ...
  const handleUserSendSupportMessage = (text: string) => {
    ...
    if (!hasTicket) {
      finalTickets = [
        ...supportTickets,
        {
          ...
          userName: name,
          userRole: role,
          messages: [{ sender: 'user' as const, text, timestamp: new Date().toISOString() }]
        }
      ];
    }
    ...
    setTimeout(() => {
      setSupportTickets(prev => prev.map(tc => {
        if (tc.userEmail === email) {
          let botReplyText = 'Your ticket was routed to active administrators. Expect an response within a few moments.';
          const lower = text.toLowerCase();
          if (lower.includes('outdated') || lower.includes('prop-')) {
            botReplyText = '🤖 Trust Inspector Guard: Analyzing listing records... Registered flag for duplication / outdated price audits.';
          } else if (lower.includes('verify') || lower.includes('deed')) {
            botReplyText = '🤖 Owner Registry Audits: Please provide county deed reference numbers inside the owner portal so admins can verify your seller profile.';
          }
          return {
            ...tc,
            messages: [
              ...tc.messages,
              { sender: 'bot' as const, text: botReplyText, timestamp: new Date().toISOString() }
            ]
          };
        }
        return tc;
    ...
    setSupportTickets(prev => prev.map(tc => {
      if (tc.userEmail === userEmail) {
        return {
          ...
          messages: [
            ...tc.messages,
            { sender: 'admin' as const, text, timestamp: new Date().toISOString() }
          ]
        };
      }
      return tc;
  ...
  const handleSaveSearch = () => {
    ...
    setCustomSearchName('');
    alert(`Search query "${newSave.name}" successfully bookmarked to database!`);
  };
  ...
  const handlePostProperty = (payload: Property) => {
    ...
    if (exists) {
      ...
      alert('Congratulations! Your listing has been published live to the PropFind marketplace.');
    }
  ...
  const handleDeleteProperty = (propertyId: string) => {
    if (window.confirm('Are you sure you want to permanently delete this listing? This operation cannot be undone.')) {
      setProperties(prev => prev.filter(p => p.id !== propertyId));
      alert('The listing has been successfully deleted from the marketplace catalog.');
    }
  };
    ...
    setProperties(prev => prev.map(p => {
      if (p.id === activePromotedPropertyId) {
        return {
          ...
          promotionExpiryDate: new Date(Date.now() + promotionDuration * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
        };
      }
      return p;
    }));

    setShowPromoteFeedback(`Success! Listing promoted to ${selectedPromotionType.toUpperCase()} tier for ${promotionDuration} days.`);
    setTimeout(() => {
  ...
  const handleModifyPropertyStatus = (propertyId: string, status: Property['status']) => {
    setProperties(prev => prev.map(p => p.id === propertyId ? { ...p, status } : p));
  };

  const handleVerifyAgentBadge = (agentId: string) => {
    setAgents(prev => prev.map(a => a.id === agentId ? { ...a, isVerified: true } : a));
  };

  const handleModeratorReportAction = (reportId: string, action: 'dismissed' | 'removed') => {
    const report = reportedListings.find(r => r.id === reportId);
    if (!report) return;

    if (action === 'removed') {
      // Set target property status to off-market
      setProperties(prev => prev.map(p => p.id === report.propertyId ? { ...p, status: 'off-market' } : p));
    }

    setReportedListings(prev => prev.map(r => r.id === reportId ? { ...r, status: action } : r));
  };

  const handleSuspendAccount = (email: string) => {
    alert(`Account associated with identifier ${email} has been given an official platform warnings flag.`);
  };

  // Add fraud report callback from details modal
  const handleCreateFraudReport = (propertyId: string, reason: string, details: string) => {
    const target = properties.find(p => p.id === propertyId);
  ...
  const handleUpdateInquiry = (inquiryId: string, updatedFields: Partial<Inquiry>) => {
    setInquiries(prev => prev.map(inq => inq.id === inquiryId ? { ...inq, ...updatedFields } : inq));
  };
  ...
  const filteredProperties = properties.filter(prop => {
    ...
    if (selectedAmenities.length > 0) {
      const matchAll = selectedAmenities.every(am => prop.amenities.includes(am));
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans" id="propfind-platform-root">
      
      {/* Dynamic top notifications for price drop warnings (UE-06) */}
      {priceDropAlerts.length > 0 && (
        <div className="bg-blue-600 p-2.5 px-4 text-center text-xs font-bold text-white flex justify-between items-center z-50 sticky top-0 animate-pulse duration-1000" id="global-news-banner">
          <div className="flex items-center gap-2 mx-auto">
            <Sparkles className="w-4 h-4 text-amber-300 animate-spin-slow" />
            <span>Price Drop Warning! &quot;{priceDropAlerts[0].title}&quot; dropped to ${priceDropAlerts[0].newPrice.toLocaleString()}! Saving you 5%</span>
          </div>
          <button onClick={() => setPriceDropAlerts([])} className="text-white hover:text-slate-200">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      <header className="bg-slate-900 border-b border-slate-800 p-4 sticky top-0 z-40 shadow-xl" id="platform-main-header">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-black text-white tracking-widest uppercase">PropFind</h1>
                <span className="text-[10px] bg-slate-950 text-slate-400 p-1 px-2.5 rounded-full border border-slate-800 font-bold tracking-tight">V1.0</span>
              </div>
              <p className="text-[10px] text-slate-400">Integrated MLS & Brokerage Ecosystem</p>
            </div>
          </div>

          {/* Persona Hot-swap Selector panel. Filtered to obscure secure staff console. (UA-02, AD-02) */}
          <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-2xl border border-slate-800 w-full md:w-auto overflow-x-auto scrollbar-none" id="persona-swapper">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider px-2 font-mono whitespace-nowrap shrink-0">Workspace:</span>
            {[
              ...
              ...(currentRole === 'admin' ? [{ role: 'admin', label: 'Plat. Admin', icon: ShieldCheck }] : [])
            ].map((p) => (
              <button
                key={p.role}
                onClick={() => {
      ...
      {!isAuthenticated && (
        <div className="bg-gradient-to-r from-blue-900 via-slate-900 to-indigo-950 border-b border-blue-900/40 p-2.5" id="guest-sandbox-header">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2 text-xs font-mono">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-300">👤 Guest Sandbox Exploration. Secret Auth Passport is requested to record favorites, list private properties, or schedule tours.</span>
            </div>
            <button
              onClick={() => {
                setAuthIntendedAction(null);
      ...
      {currentRole === 'admin' && (
        <div className="bg-amber-955/20 border-b border-amber-900/40 p-2 text-center text-xs text-amber-400 font-mono" id="staff-auth-banner">
          <div className="max-w-7xl mx-auto flex justify-between items-center px-4">
            <span className="flex items-center gap-1.5">🔒 <strong>Staff Office Gateway</strong>: Authenticated Session Active. Secure Moderation Terminal decrypted.</span>
            <button
              type="button"
              onClick={() => {
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 md:p-6" id="primary-view-container">
        ...
        {currentRole === 'seeker' && (
          <div className="space-y-6" id="seeker-workspace">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="seeker-intro-bar">
              <div className="md:col-span-2 bg-slate-900 rounded-3xl p-5 border border-slate-800 flex flex-col justify-center relative overflow-hidden perspective-1200 transform-style-3d">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 mb-1.5 pr-[120px] md:pr-0">
                  <span className="text-xs text-blue-400 font-mono uppercase tracking-widest font-bold">Discover Luxury Real Estate</span>
                  {isAuthenticated ? (
                    <span className="text-[9px] bg-emerald-950 text-emerald-400 border border-emerald-900/60 font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded">
                      Passport Verified (Age {userProfile.role === 'seeker' && userProfile.email === 'chibuikeeseagwu02@gmail.com' ? 34 : 30})
                    </span>
                  ) : (
                    <span className="text-[10px] bg-slate-950 text-slate-500 font-mono border border-slate-850 px-2 py-0.5 rounded">Temporary Guest Pass</span>
                  )}
                </div>
                <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight mt-1">Hello, {userProfile.name}</h2>
                <p className="text-xs text-slate-400 mt-1 max-w-xl">
                  Filter through curated high-res listings, saved parameters, compare housing values, or draw custom boundary areas inside the Interactive Map to find your future home.
                </p>

                {/* Demographic Trust Playbook Advices (US-08) */}
                <div className="mt-3 p-3 bg-slate-950/70 rounded-xl border border-slate-850 text-xs text-slate-300 space-y-1.5" id="demographics-playbook-widget">
                  <div className="flex items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-blue-400" />
                    <span className="text-[10px] font-bold text-white uppercase tracking-wider font-mono">My Persona Playbook Advice</span>
                  </div>
                  {isAuthenticated ? (() => {
                    // Custom seeker age is typically around 34
                    const isChibuike = userProfile.email === 'chibuikeeseagwu025@gmail.com' || userProfile.email === 'chibuikeeseagwu02@gmail.com';
                    const userAge = isChibuike ? 34 : 30; // Use realistic demographic numbers
                    if (userAge <= 35) {
                      return (
                        <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                          🎯 <strong>First-Time Seeker (Age {userAge})</strong>: Look for high-contrast verified listings in Marina Heights. To avoid outdated listings or scammers, consult our county-recorder approved private sellers (like George Clooney).
                        </p>
                      );
                    } else if (userAge > 35 && userAge <= 55) {
                      return (
                        <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                          🎯 <strong>Family Relocator (Age {userAge})</strong>: Leverage school ratings (10/10) filters and utilize the <strong>Map Lasso tool</strong> (draw custom boundaries) to align with school district perimeters.
                        </p>
                      );
                    } else {
                      return (
                        <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                          🎯 <strong>Investor Segment (Age {userAge})</strong>: Audit commercial leases or office metrics, and leverage our <strong>simulated price drop triggers</strong> to analyze historical value adjustments.
                        </p>
                      );
                    }
                  })() : (
                    <p className="text-[11px] text-slate-400 font-sans italic">
                      ⚙️ Welcome, guest seeker! Sign in to automatically match your account demographics with county-certified real estate suggestions.
                    </p>
                  )}
                </div>
                ...
                <div className="mt-4 relative max-w-md" id="search-bar-parent">
                  ...
                  {showSuggestions && filteredSuggestions.length > 0 && (
                    <div className="absolute left-0 right-0 mt-1.5 bg-slate-900 border border-slate-850 rounded-xl shadow-2xl z-50 overflow-hidden font-mono" id="autocomplete-suggestions">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex flex-col justify-between" id="quick-bookmarks-indices">
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest font-mono">Workspace Index</span>
                  <h4 className="text-white font-semibold text-xs leading-tight">My Active Favorites Dashboard</h4>
                  <p className="text-[11px] text-slate-400">Total saves: {userProfile.savedProperties.length} items</p>
                </div>

                <div className="flex gap-2 mt-4" id="view-matrix-compare-trigger">
                  <button
                    type="button"
                    onClick={() => {
                      // Anchor comparative table
                      const element = document.getElementById('comparison-matrix-anchor');
                  ...
                  >
                    ...
                    <span>View Compare Matrix ({comparisonPropertyIds.length})</span>
                  </button>
                </div>
              </div>

              {/* Dynamic Recently Viewed Panel */}
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex flex-col justify-between" id="seeker-recently-viewed-panel">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-blue-400 font-bold uppercase tracking-widest font-mono">Recent History</span>
                    <span className="text-[9px] bg-slate-955 px-2 py-0.5 rounded text-slate-400 border border-slate-850 font-mono font-semibold">{recentlyViewedProperties.length}/5</span>
                  </div>
                  <h4 className="text-white font-semibold text-xs leading-tight">Recently Viewed</h4>
                  ...
                  {recentlyViewedProperties.length === 0 ? (
                    <div className="text-center p-4 border border-dashed border-slate-800 rounded-2xl bg-slate-950/40">
                      <p className="text-[11px] text-slate-500 italic">No views register yet. Toggle properties to populate.</p>
                    </div>
                  ) : (
                    <div className="space-y-2 mt-1 max-h-[140px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-800" id="recently-viewed-inner-scroll">
                      {recentlyViewedProperties.map((p) => (
                        ...
                        >
                          <img src={p.photos[0]} alt={p.title} className="w-8 h-8 rounded-lg object-cover border border-slate-800 shrink-0" />
                          <div className="min-w-0 flex-1">
                            <h5 className="text-[10px] font-bold text-slate-200 truncate leading-tight font-display">{p.title}</h5>
                            <p className="text-[9px] text-slate-400 font-mono">${p.price.toLocaleString()}{p.listingType === 'rent' ? '/mo' : ''}</p>
                          </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-4 shadow-md" id="search-parameters-deck">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4" id="seeker-toolbar-header">
                <div className="grid grid-cols-2 md:flex md:items-center gap-3 w-full lg:w-auto" id="essential-filters-row">
                  
                  {/* Sector Region Select */}
                  <div className="flex flex-col gap-1 col-span-1">
                    <span className="text-[9px] text-slate-500 uppercase font-bold font-mono">Market Region</span>
                    <select
                      value={selectedCity}
                  ...
                  <div className="flex flex-col gap-1 col-span-2 md:col-span-1">
                    <span className="text-[9px] text-slate-500 uppercase font-bold font-mono">Contract Type</span>
                  ...
                  {/* Sorting dropdown */}
                  <div className="flex flex-col gap-1 col-span-1">
                    <span className="text-[9px] text-slate-500 uppercase font-bold font-mono">Sort Catalog</span>
                    <select
                      value={sortingOption}
                      onChange={(e) => setSortingOption(e.target.value)}
                      className="bg-slate-950 border border-slate-800 rounded-lg text-xs p-2 px-3 focus:outline-none focus:border-blue-500 text-white min-h-[38px] cursor-pointer"
                    >
                      <option value="relevance">Popular &amp; High Views</option>
                ...
                <div className="flex flex-wrap gap-2 items-center justify-between md:justify-end w-full lg:w-auto" id="seekers-controls-tools">
              ...
              {showAdvancedFilters && (
                <div className="border-t border-slate-850 pt-4 grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs animate-fade-in" id="panel-advanced-filters">
                  
                  {/* Property Type Category */}
                  <div className="space-y-1">
                    <label className="block text-[10px] text-slate-400 uppercase font-bold tracking-tight">Category Type</label>
                    <select
                  ...
                  {/* Sizing Prices */}
                  <div className="space-y-1">
                    <label className="block text-[10px] text-slate-400 uppercase font-bold tracking-tight">Price limits ($)</label>
                  ...
                  {/* Bed counts */}
                  <div className="space-y-1">
                    <label className="block text-[10px] text-slate-400 uppercase font-bold tracking-tight">Beds count</label>
                    <select
                  ...
                  {/* Listing Status */}
                  <div className="space-y-1">
                    <label className="block text-[10px] text-slate-400 uppercase font-bold tracking-tight">Listing Status</label>
                    <select
                      value={statusFilter}
                  ...
                  <div className="space-y-1 flex flex-col justify-end">
                    <label className="block text-[10px] text-slate-400 uppercase font-bold tracking-tight mb-1">Save Search Filter</label>
            ...
            {viewMode === 'all' ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start animate-entrance-3d-effect" id="seekers-split-pane">
                {/* 3D Map Anchor Left Column */}
                <div className="lg:col-span-4 xl:col-span-5 h-[320px] sm:h-[480px] lg:h-[550px] lg:sticky lg:top-24 z-10" id="map-sticky-container">
                  <InteractiveMap
                  ...
                  <div className="mt-2 text-[10px] text-slate-500 font-mono text-center">
                    💡 Click <strong>&quot;Draw Search Area&quot;</strong> in map header to isolate desirable plots.
                  </div>
                </div>

                {/* Listing Grid Scrolling Right Column */}
                <div className="lg:col-span-8 xl:col-span-7 space-y-4" id="cards-scrolling-container">
                  {/* Stats & Search counts */}
                  <div className="flex flex-wrap items-center justify-between gap-3 text-xs border-b border-slate-900 pb-2" id="seekers-counters-drawer-split">
                    <span className="font-semibold text-slate-400">
                      Showing <strong className="text-white">{sortedProperties.length}</strong> matching premium properties
                    </span>

                    {savedSearches.length > 0 && (
                      <div className="flex items-center gap-1.5" id="saved-searches-drawer-split">
                        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-mono">Saved searches:</span>
                  ...
                  {sortedProperties.length === 0 ? (
                    <div className="text-center p-12 bg-slate-900/40 border border-slate-800 rounded-3xl" id="seekers-search-empty-split">
                      <SlidersHorizontal className="w-8 h-8 mx-auto text-slate-600 mb-2" />
                      <p className="text-sm text-slate-450 font-semibold">Zero Matching Listings found</p>
                      <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">Your advanced parameters are too constrained. Reset filters to reload catalog.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" id="seekers-results-grid-split">
                      {sortedProperties.map((prop) => {
                        const isSaved = userProfile.savedProperties.includes(prop.id);
                          ...
                          >
                            ...
                            <div className="relative h-44 bg-slate-950 overflow-hidden">
                              ...
                              {(prop.isPromoted || prop.isFeatured) && (
                                <span className={`absolute top-2.5 left-2.5 text-slate-950 font-black text-[9px] uppercase tracking-wider px-2 py-0.5 rounded shadow-lg flex items-center gap-1 ${
                                  prop.isFeatured ? 'bg-amber-400 border border-amber-300 animate-pulse' : 'bg-blue-400 border border-blue-300'
                                }`}>
                                  ★ {prop.isFeatured ? 'FEATURED' : (prop.promotionType?.toUpperCase() || 'PROMOTED')}
                                </span>
                              )}

                              <span className={`absolute bottom-2.5 left-2.5 backdrop-blur-md shadow-lg font-bold font-mono text-[9px] uppercase tracking-wider px-2.5 py-1 rounded-lg border ${
                                prop.status === 'active' ? 'bg-emerald-950/90 text-emerald-400 border-emerald-900/60' :
                                prop.status === 'pending' ? 'bg-amber-950/90 text-amber-400 border-amber-900/60' :
                                prop.status === 'sold' ? 'bg-blue-950/90 text-blue-400 border-blue-900/60' :
                                prop.status === 'rented' ? 'bg-purple-950/90 text-purple-400 border-purple-900/60' :
                                prop.status === 'off-market' ? 'bg-slate-950/90 text-slate-400 border-slate-800' :
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-entrance-3d-effect" id="seekers-map-split">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between" id="map-quick-guide">
                  <div className="space-y-3">
                    <span className="text-[10px] text-blue-400 font-bold tracking-widest font-mono uppercase">Map Projection HUD</span>
                    <h5 className="font-bold text-white text-xs">How to use Map Coordinates:</h5>
                    <p className="text-[11px] text-slate-400 leading-relaxed font-mono">
                      1. Click &quot;Draw Search Area&quot; in the map header.<br />
                      2. Draw a complete loop on the grid to bind coordinating plots.<br />
                      3. Properties within bounds are selected immediately below!<br />
                      4. Hover on coordinates markers to preview luxury highlights.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 animate-entrance-3d-effect" id="seekers-catalog">
                
                {/* Active results count row and Saved Searches index drawer */}
                <div className="flex flex-wrap items-center justify-between gap-3 text-xs border-b border-slate-900 pb-2" id="seekers-counters-drawer">
                  <span className="font-semibold text-slate-400">
                    Showing <strong className="text-white">{sortedProperties.length}</strong> matching premium properties
                  </span>

                  {savedSearches.length > 0 && (
                    <div className="flex items-center gap-1.5" id="saved-searches-drawer">
                      <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-mono">My Saved Searches:</span>
                ...
                {/* Empty check */}
                {sortedProperties.length === 0 ? (
                  <div className="text-center p-12 bg-slate-900/40 border border-slate-800 rounded-3xl" id="seekers-search-empty">
                    <SlidersHorizontal className="w-8 h-8 mx-auto text-slate-600 mb-2" />
                    <p className="text-sm text-slate-450 font-semibold">Zero Matching Listings found</p>
                    <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">Your advanced parameters are too constrained. Try resetting filters to reload.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="seekers-results-grid">
                    {sortedProperties.map((prop) => {
                      const isSaved = userProfile.savedProperties.includes(prop.id);
                      const isCompare = comparisonPropertyIds.includes(prop.id);
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Body parameters */}
                          <div className="p-4 space-y-1.5 flex-1 flex flex-col justify-between" id={`card-body-${prop.id}`}>
                            <div>
                              <div className="flex justify-between items-start gap-2">
                                <h4
                                  onClick={() => triggerPropertyDetail(prop.id)}
                            ...
                            <div className="pt-3.5 space-y-2.5 border-t border-slate-950 mt-3" id={`card-footer-triggers-${prop.id}`}>
                              {/* Standard specs */}
            ...
            {userProfile.savedProperties.length > 0 && (
              <div className="bg-slate-900/50 rounded-3xl p-5 border border-slate-800 space-y-4" id="notes-persistency-desk">
                <div id="notes-label">
                  <span className="text-xs text-blue-400 font-mono uppercase tracking-widest font-bold">Personal Workspace</span>
                  <h3 className="text-sm font-bold text-white mt-0.5">My Favorited Private Notes</h3>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" id="fav-notes-grid">
                  {properties.filter(p => userProfile.savedProperties.includes(p.id)).map((fav) => (
                    <div key={fav.id} className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
                      <div className="space-y-1">
                        <span className="text-[10px] text-blue-400 font-bold">{fav.title}</span>
                        <p className="text-[10px] text-slate-400 font-mono">My personal comments:</p>
                        <p className="text-[11px] text-slate-300 leading-normal italic">
                          &quot;{userProfile.notesOnProperties[fav.id] || 'No notes currently stored for this property.'}&quot;
                        </p>
                      </div>
                      ...
                      <div className="flex gap-2 pt-2 border-t border-slate-900">
                        <input
                          ...
                          onChange={(e) => setTempNoteText({ ...tempNoteText, [fav.id]: e.target.value })}
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            updatePersonalNote(fav.id, tempNoteText[fav.id] || '');
                            setTempNoteText({ ...tempNoteText, [fav.id]: '' });
                          }}
            ...
            <div className="pt-6 border-t border-slate-900" id="comparison-matrix-anchor">
              <span className="text-xs text-blue-400 font-mono uppercase tracking-widest block font-bold mb-3">Feature Comparison Cockpit</span>
              <PropertyCompare
                properties={properties.filter(p => comparisonPropertyIds.includes(p.id))}
        ...
        {currentRole === 'owner' && (
          <div className="space-y-6" id="owner-workspace">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4" id="owner-summary-header">
              <div className="space-y-1">
                <span className="text-xs text-blue-400 font-mono uppercase tracking-widest font-bold">Seller hub workspace</span>
                <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">Post &amp; Track Private Real Estate</h2>
                <p className="text-xs text-slate-400 max-w-xl">
                  Register details, upload mock picture streams, set leasing status updates, or analyze listing views.
                </p>
              </div>
            ...
            {(isCreatingListing || editingProperty) ? (
              ...
              <div className="space-y-6" id="owner-portfolio-grid">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3" id="owners-portfolio-highlights">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest font-mono">My Active Listings performance summary</span>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4" id="owned-metrics-panel">
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-850">
                      <span className="text-slate-400 text-xs">Platform Page Views</span>
                      <p className="text-white text-2xl font-black mt-1">1,910 views</p>
                      <p className="text-[10px] text-emerald-400 mt-1 font-mono">+12.4% vs last week</p>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-850">
                      <span className="text-slate-400 text-xs">Total client saves</span>
                      <p className="text-white text-2xl font-black mt-1">362 bookmarked</p>
                      <p className="text-[10px] text-blue-400 mt-1 font-mono">Average item: 42 saves</p>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-850">
                      <span className="text-slate-400 text-xs">Pending tour Requests</span>
                      <p className="text-white text-2xl font-black mt-1">{inquiries.length} requests</p>
                      <p className="text-[10px] text-amber-500 mt-1 font-mono">100% Client Response rate</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4" id="owned-listings-catalog-grid">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 font-mono">My properties listings ({properties.length})</h4>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4" id="owned-cards-grid">
                    {properties.map((p) => (
                      <div key={p.id} className="p-4 bg-slate-900 border border-slate-800 rounded-2xl flex flex-col md:flex-row justify-between gap-4 hover-perspective-card" id={`owned-item-${p.id}`}>
                        <div className="flex gap-3">
                          <img src={p.photos[0]} className="w-20 h-20 rounded-lg object-cover border border-slate-800" />
                          <div>
                            <h5 className="font-bold text-white text-xs leading-normal">{p.title}</h5>
                            <p className="text-[10px] text-slate-400 font-mono">{p.address} &bull; {p.city}</p>
                            <p className="text-blue-400 font-bold text-[11px] font-mono mt-1">${p.price.toLocaleString()}</p>
                            
                            <div className="flex gap-2 items-center mt-2" id={`owned-tags-row-${p.id}`}>
                              <span className={`px-2 py-0.5 rounded text-[8px] tracking-wider uppercase font-bold border ${
                                p.status === 'active' ? 'bg-emerald-950 text-emerald-400 border-emerald-900/60' :
                                p.status === 'pending' ? 'bg-amber-950 text-amber-400 border-amber-900/60' :
                                p.status === 'sold' ? 'bg-blue-950 text-blue-400 border-blue-900/60' :
                                p.status === 'rented' ? 'bg-purple-950 text-purple-400 border-purple-900/60' :
                                p.status === 'off-market' ? 'bg-slate-900 text-slate-400 border-slate-800' :
                              ...
                              {p.autoRenewBeforeExpiry && (
                                <span className="text-slate-400 text-[8px] font-bold px-1.5 py-0.5 rounded border border-slate-800 bg-slate-950 uppercase tracking-wider font-mono" title="Auto Renewal Active">
                                  🔄 Auto-Renew
                                </span>
                              )}
                              <span className="text-[9px] text-slate-500 font-mono">Views: {p.views}</span>
                            </div>
                          </div>
                        </div>

                        {/* Interactive listing management triggers (LM-04, LM-05, LM-06, LM-10) */}
                        <div className="flex flex-col gap-2 justify-center" id={`owned-triggers-${p.id}`}>
                          <button
                            type="button"
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              alert('Demonstration price drop registered under local state, notifications dispatched!');
                            }}
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4" id="owner-reputation-metrics">
                  <div className="flex items-center justify-between border-b border-slate-800/60 pb-3" id="owner-rep-header">
                    <div>
                      <span className="text-[10px] text-amber-500 font-mono uppercase tracking-widest block font-bold">Verified Landlord Reputation Certificate</span>
                      <h4 className="text-white font-black text-sm tracking-tight">My Public Trust Grade & Anti-Scam Verification</h4>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4" id="rep-benchmarks-grid">
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-850 space-y-1">
                      <span className="text-slate-500 text-[10px] uppercase font-bold font-mono block">Scam Audit Status</span>
                      <p className="text-emerald-400 font-bold text-xs">🛡️ 100% SECURED</p>
                      <p className="text-[10px] text-slate-400 leading-normal font-sans">County deed registries successfully cross-checked with official land records.</p>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-850 space-y-1">
                      <span className="text-slate-500 text-[10px] uppercase font-bold font-mono block">Image Authenticity</span>
                      <p className="text-blue-400 font-bold text-xs">📸 PHOTO-VERIFIED</p>
                      <p className="text-[10px] text-slate-400 leading-normal font-sans">No stock photos or outdated images. Preview assets checked by site audits.</p>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-850 space-y-1">
                      <span className="text-slate-500 text-[10px] uppercase font-bold font-mono block">Dispute Resolution</span>
                      <p className="text-purple-400 font-bold text-xs">🤝 TRUSTED PARTNER</p>
                      <p className="text-[10px] text-slate-400 leading-normal font-sans">Zero formal reports or complaints from property seekers since joining.</p>
                    </div>
                  </div>

                  {/* Customer Testimonials stream (US-06) */}
                  <div className="space-y-2.5 pt-2" id="verified-testimonials-stack">
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider font-mono">Verified Buyer & Tenant Reviews (3):</span>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3" id="testimonials-list">
                      {[
                        { author: "Alice M.", text: "Pristine listing experience. The coordinates and floor designs matched the physical house perfectly. Highly responsive landlord!", stars: "★★★★★" },
                        { author: "Ken T.", text: "George provided the authentic county tax paperwork promptly. Absolutely zero scam concern! Fully recommend to any relocation seekers.", stars: "★★★★☆" },
                        { author: "Marcus V.", text: "Clean and direct interaction. Secured the Marina Heights glass penthouse in 3 days. PropFind guard verified his ownership badge.", stars: "★★★★★" }
                      ].map((t, idx) => (
                        <div key={idx} className="p-3 bg-slate-950 rounded-xl border border-slate-850/60 text-xs">
                          <div className="flex justify-between items-center mb-1">
                            <span className="font-bold text-slate-200">{t.author}</span>
                            <span className="text-amber-500 font-mono text-[9px] font-extrabold">{t.stars}</span>
                          </div>
                          <p className="text-slate-400 italic text-[11px] leading-relaxed">&quot;{t.text}&quot;</p>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {currentRole === 'agent' && (
          <div className="space-y-6" id="agent-workspace">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4" id="agents-context-header">
              <div className="space-y-1">
                <span className="text-xs text-amber-500 font-mono uppercase tracking-widest font-bold">Broker Core Workspace</span>
                <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">Vanguard Brokerage Platform</h2>
                <p className="text-xs text-slate-400 max-w-xl">
                  Inspect client leads, configure chat correspondence, approve tours calendars, or boost asset visibility listings.
                </p>
              </div>

              {/* Show active broker stats summaries (AG-07) */}
              <div className="p-3 bg-slate-950 border border-slate-850 rounded-xl text-center self-start sm:self-auto" id="agents-performance-badge-view">
                <span className="text-[9px] tracking-wider text-slate-500 uppercase font-bold font-mono block">Aggregate Agency Performance</span>
                <div className="flex gap-4 mt-1 font-mono" id="ag-aggregate-metrics">
                  <div>
                    <span className="text-[10px] text-slate-400 block font-mono">Sold:</span>
                    <strong className="text-white text-xs">{INITIAL_AGENTS[0].performance.propertiesSold} items</strong>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block font-mono">Response:</span>
                    <strong className="text-emerald-400 text-xs">{INITIAL_AGENTS[0].performance.responseRate}%</strong>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3" id="crm-desktop-nest">
              <span className="text-xs text-blue-400 font-mono uppercase tracking-widest block font-bold">Broker leads CRM Cockpit</span>
              <InboxChat
                inquiries={inquiries}
            ...
            <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl space-y-4" id="listing-boosts-section">
              <div id="booster-caption">
                <span className="text-xs text-blue-400 font-mono uppercase tracking-widest font-bold font-mono">Revenue & Promotion desks</span>
                <h3 className="text-sm font-bold text-white mt-0.5">Promote Premium Real Estate listings</h3>
                <p className="text-xs text-slate-400">Increase search reach, place units inside spotlight areas, and verify placement metrics.</p>
              </div>

              {activePromotedPropertyId ? (
                /* Boost payment form setup */
                <form onSubmit={handleApplyPromotion} className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-4 max-w-lg" id="promotion-checkout-form">
                  <span className="text-[10px] text-amber-500 font-bold uppercase font-mono tracking-wider">Configure Promotional Placement Checkout</span>
                  
                  <div className="grid grid-cols-2 gap-3 text-xs" id="boost-form-selections">
                    <div>
                      <label className="block text-slate-400 mb-1">Duration Period</label>
                      <select
                    ...
                    <div>
                      <label className="block text-slate-400 mb-1">Exposure Level</label>
                      <select
                        value={selectedPromotionType}
                        onChange={(e) => setSelectedPromotionType(e.target.value as any)}
                  ...
                  <div className="bg-slate-900 p-2.5 rounded text-[11px] font-mono flex justify-between border border-slate-850" id="pricing-math-boost">
                    <span className="text-slate-400">Total charge:</span>
                    <strong className="text-emerald-400">${promotionDuration * (selectedPromotionType === 'premium' ? 15 : 8)} USD</strong>
                  </div>

                  {showPromoteFeedback && (
                    <p className="text-xs text-emerald-400 bg-emerald-950 p-2 rounded text-center animate-pulse">{showPromoteFeedback}</p>
                  )}

                  <div className="flex gap-2 justify-end text-xs" id="boost-actions-checkout">
                    <button
                      type="button"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" id="eligible-promo-listings">
                  {properties.map((p) => (
                    <div key={p.id} className="p-3 bg-slate-950 rounded-xl border border-slate-850 flex justify-between items-center gap-4">
                      <div>
                        <h6 className="text-xs font-bold text-white leading-normal">{p.title}</h6>
                        <span className="text-[10px] text-slate-400 font-mono">Region: {p.city} &bull; ${p.price.toLocaleString()}</span>
                      </div>
      ...
      {selectedPropertyId && (
        (() => {
          ...
          const matchedAgent = agents.find(ag => ag.id === matchedProp.agentId) || agents[0];
          ...
          return (
            <PropertyDetailModal
              ...
              onReportListing={(reason, details) => handleCreateFraudReport(matchedProp.id, reason, details)}
            />
          );
        })()
      )}
      ...
      <SupportChatbot
        ...
        activeTicketMessages={supportTickets.find(t => t.userEmail === userProfile.email)?.messages || [
          { sender: 'bot', text: 'Hello! I am PropFind Guard, your automatic Trust & Safety desk. Ask any question or report duplicate listings to get admin assistance!', timestamp: new Date().toISOString() }
        ]}
      ...
      {isAdminLockScreenVisible && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 animate-fade-in" id="staff-lockscreen-overlay">
          <div className="bg-slate-900 border-2 border-slate-800 rounded-3xl p-6 max-w-sm w-full space-y-4 shadow-2xl relative">
            ...
            <div className="text-center space-y-1.5">
              ...
              <h3 className="text-white font-black text-sm uppercase tracking-wider font-mono">Decryption Staff Challenge</h3>
              <p className="text-[10px] text-slate-400 font-sans">Verification key is required to render active Moderator tickets.</p>
            </div>

            <div className="space-y-3">
              <div>
                <label className="block text-[8px] text-slate-500 uppercase font-bold font-mono">Administrative Password</label>
                <input
                  type="password"
                  placeholder="••••••••"
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      if (enteredPasscode === 'admin123') {
                        ...
                        setPasscodeError('Decryption failure: Invalid staff passcode key');
                      }
                    }
                  }}
                />
                {passcodeError && (
                  <p className="text-[9px] text-red-400 mt-1 font-mono text-center">{passcodeError}</p>
                )}
              </div>

              <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-850 text-center text-[9px] text-slate-500 font-mono">
                Hint: Enter <code className="text-amber-400 font-bold">admin123</code> to authenticate Supervisor credentials.
              </div>
      ...
      <footer className="bg-slate-900 border-t border-slate-800 p-6 mt-12 text-xs text-slate-500 font-mono" id="platform-global-footer">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="order-2 md:order-1">&copy; 2026 PropFind Real Estate Inc. All rights reserved.</p>
          <div className="flex flex-wrap gap-4 order-1 md:order-2 text-[11px] items-center" id="policy-links">
            <span className="hover:text-white cursor-pointer">Security Audits</span>
            <span className="hover:text-white cursor-pointer">Listing Guidelines</span>
            <span className="hover:text-white cursor-pointer">Broker license terms</span>
            <button
              onClick={() => {
