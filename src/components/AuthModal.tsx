...
import { X, ShieldCheck, Mail, User, Lock, Fingerprint, ShieldAlert, CheckSquare } from 'lucide-react';

interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAuthenticate: (userData: { name: string; email: string; age: number; role: 'seeker' | 'owner' | 'agent' | 'admin' }) => void;
  initialMode?: 'signin' | 'signup';
}

const DUMMY_SHORTCUTS = [
  { name: 'Chibuike Eseagwu', email: 'chibuikeeseagwu02@gmail.com', age: 34, role: 'seeker', label: 'Chibuike (34, Seeker)' },
  { name: 'George Clooney', email: 'george.clooney@hollywood.com', age: 50, role: 'owner', label: 'George (50, Private Owner)' },
  { name: 'Sarah Jenkins', email: 'sarah.jenkins@propfind.com', age: 38, role: 'agent', label: 'Sarah (38, Agency Broker)' }
];

export default function AuthModal({ isOpen, onClose, onAuthenticate, initialMode = 'signin' }: AuthModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      ...
      if (!formData.nin) {
        setError('NIN (National Identification Number) is compulsory for anti-scam identity binding.');
        return;
      }
      ...
      if (!formData.agreeAntiScam) {
        setError('You must attest and agree to the double-lock anti-scam disclosure.');
        return;
      }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto" id="auth-wall-modal">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md my-8 text-slate-100 shadow-2xl relative p-6 space-y-5" id="auth-panel">
        ...
        <div className="text-center space-y-1" id="auth-hdr">
          ...
          <h3 className="font-extrabold text-white text-lg tracking-tight">PropFind Member Gate</h3>
        ...
        <div className="p-3 bg-slate-950 border border-slate-850 rounded-xl space-y-2" id="shortcut-box">
          <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block font-mono">Sandbox Quick Sign-In Profiles:</span>
          <div className="grid grid-cols-1 gap-1.5" id="shortcuts-grid">
            {DUMMY_SHORTCUTS.map((sc, idx) => (
              ...
              >
                <span>👤 {sc.label}</span>
                <span className="text-[9px] bg-slate-950 text-blue-400 font-bold uppercase px-1.5 py-0.2 rounded border border-slate-850">Quick-Load</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="relative flex py-1 items-center" id="or-separator">
          ...
          <span className="flex-shrink mx-3 text-[9px] font-mono text-slate-600 uppercase">Or Key In Details</span>
          <div className="flex-grow border-t border-slate-850"></div>
        </div>

        {error && (
          <div className="p-2.5 bg-red-950/40 border border-red-900 text-red-400 text-xs rounded-xl font-medium" id="auth-err">
            ⚠️ {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4" id="custom-auth-form">
          ...
          <div>
            <label className="block text-[10px] text-slate-400 font-bold uppercase mb-1">Account Password</label>
            <div className="relative">
          ...
          {mode === 'signup' && (
            <div className="space-y-4 pt-1 border-t border-slate-850 mt-1" id="signup-enhanced-security-fields">
              ...
              <div className="grid grid-cols-2 gap-3" id="signup-role-age-grid">
                <div>
                  ...
                  />
                  <span className="text-[9px] text-slate-500 font-mono italic mt-1 block">Demographic checks apply</span>
                </div>

                <div>
                  <label className="block text-[10px] text-slate-400 font-bold uppercase mb-1">Operational Role</label>
                  <select
                    value={formData.role}
              ...
              <div>
                ...
                <span className="text-[8px] text-slate-500 font-mono mt-1 block">
                  Encrypted securely with 256-bit protocol for landlord tax registry.
                </span>
              </div>
              ...
              <label className="flex items-start gap-2 text-[10px] text-slate-300 font-medium cursor-pointer" id="anti-scam-attestation-checkbox-label">
                <input
                  type="checkbox"
                  checked={formData.agreeAntiScam}
                  onChange={(e) => setFormData({ ...formData, agreeAntiScam: e.target.checked })}
                  className="mt-0.5 rounded border-slate-800 text-blue-600 focus:ring-blue-500 cursor-pointer"
                />
                <span>
                  I legally attest that my listing queries represent true land titles. I agree to land coordinate and double-lock audit rules.
                </span>
              </label>
          ...
          <button
            ...
            id="auth-submit-btn"
          >
            {mode === 'signin' ? 'Verify Secret Entry' : 'Register Secure Profile'}
          </button>
        </form>

        {/* Footer controls */}
        <div className="text-center font-mono text-[10px] text-slate-500 pt-1" id="auth-ftr">
          {mode === 'signin' ? (
            <p>
