...
import { ChevronRight, ChevronLeft, Upload, Check, Home, Eye, Sparkles, MapPin, DollarSign, Image, X, Play, Video, CreditCard, Calendar, RotateCcw, ShieldCheck, Info } from 'lucide-react';
...
const AVAILABLE_AMENITIES = [
  'Swimming Pool', 'Infinity Pool', 'Smart Automation', 'Wine Cellar', 'Elevator', 
  'Private Gym', 'Steam Shower', 'Walk-in Closets', 'Private Boat Slip',
  'Timber Architecture', 'Thermal Outdoor Pool', 'Rooftop Lounge', '24/7 Gated Security'
];
...
export default function ListingForm({
  ...
  const [listingType, setListingType] = useState<'buy' | 'rent' | 'lease'>('buy');
  const [propertyType, setPropertyType] = useState<'house' | 'apartment' | 'condo' | 'land' | 'commercial' | 'office'>('house');
  ...
  const [status, setStatus] = useState<'active' | 'pending' | 'sold' | 'rented' | 'off-market' | 'expired'>('active');
  const [expiryDate, setExpiryDate] = useState<string>('');
  const [autoRenewBeforeExpiry, setAutoRenewBeforeExpiry] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (editingProperty) {
      ...
      setExpiryDate(editingProperty.expiryDate || (() => {
        const d = new Date();
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const payload: Property = {
      ...
      amenities: amenities.length > 0 ? amenities : ['Central AC', 'Smart System'],
      ...
      createdDate: editingProperty ? editingProperty.createdDate : new Date().toISOString().split('T')[0],
      priceHistory: editingProperty 
        ? [...editingProperty.priceHistory, { date: new Date().toISOString().split('T')[0], price: Number(price) }]
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl text-slate-100" id="property-wizard-form">
      {/* Wizard Header bar */}
      <div className="p-5 bg-slate-950 border-b border-slate-800 flex justify-between items-center" id="wizard-bar">
        <div>
          <span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest font-mono">
            {editingProperty ? 'Edit Property Registry' : 'Publish Property Listing'}
          </span>
          <h2 className="text-lg font-bold text-white tracking-tight flex items-center gap-1.5 mt-0.5">
            <Home className="w-5 h-5 text-blue-500" />
            <span>{editingProperty ? `Update: ${title}` : 'List Your Real Estate'}</span>
          </h2>
        </div>
        <div className="flex gap-2">
          ...
          >
            ...
            <span>{isPreviewMode ? 'View Form Editor' : 'Instant Web Preview'}</span>
          </button>
          <button
            type="button"
            onClick={onCancel}
      ...
      {!isPreviewMode && (
        <div className="px-5 py-3.5 bg-slate-900/60 border-b border-slate-800 flex justify-between text-xs font-mono" id="wizard-progress-bar">
          {[
      ...
      {isPreviewMode ? (
        ...
        <div className="p-6 bg-slate-950" id="live-preview-workspace">
          <div className="max-w-md mx-auto bg-slate-900 rounded-xl overflow-hidden border border-slate-800 shadow-2xl">
            <div className="relative h-48 bg-slate-800">
              ...
              <span className="absolute bottom-2.5 right-2 rounded-lg bg-slate-950/90 backdrop-blur-sm px-2 py-1 text-blue-300 font-bold font-mono text-xs shadow-lg">
                ${price ? Number(price).toLocaleString() : '0'}{listingType === 'rent' ? '/mo' : ''}
              </span>
            </div>
            ...
            <div className="p-4 space-y-2">
              ...
              <p className="text-[11px] text-slate-400 flex items-center gap-1">
                ...
                <span>{address || 'Plot 4, Emerald Vista High Road'}, {city}</span>
              </p>

              <p className="text-xs text-slate-400 line-clamp-2 italic pt-1 mb-2">
                {description || 'Provide detailed rich text highlights within step 1 of the builder.'}
              </p>
              ...
              {amenities.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2.5">
                  {amenities.map((am) => (
                    <span key={am} className="text-[9px] bg-blue-950 border border-blue-900/40 text-blue-300 px-1.5 py-0.5 rounded-md font-medium">
                      {am}
                    </span>
                  ))}
                </div>
          ...
          <p className="text-center text-[10px] text-slate-500 font-mono mt-4">
            * This displays a streamlined card rendering representation for property seekers.
          </p>
        </div>
      ) : (
        /* Real editor Form Steps content */
        <form onSubmit={handleFormSubmit} className="p-6 space-y-6" id="wizard-form-data">
          {step === 1 && (
            <div className="space-y-4 animate-fade-in" id="wizard-step-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Listing Identifier Title *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Broker Manager Assigned</label>
                  <select
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">Listing Type</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">Property Category</label>
                  <select
                    value={propertyType}
                ...
                <div className="col-span-2 sm:col-span-1">
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Sale/Rental Price *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">Market Sector Region</label>
                  <select
                    value={city}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Detailed Listing Overview Description</label>
                <textarea
                  rows={4}
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 animate-fade-in" id="wizard-step-2">
              <div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Property Size (Sq. Ft.)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Lot Area Acreage</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Parking bays</label>
                  <input
                    type="number"
                    value={parkingSpaces}
            ...
            <div className="space-y-6 animate-fade-in" id="wizard-step-3">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <div>
                    <label className="block text-xs font-semibold text-slate-300">Property Media Gallery Selection</label>
                    <p className="text-[10px] text-slate-400">
                      Toggle preset architectural items below or perform a direct drag-over-drop of your custom pictures to list them in the active album.
                    </p>
                  </div>
                </div>

                {/* Preset Picker */}
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 bg-slate-950/20 p-3 rounded-xl border border-slate-850" id="pre-vetted-image-picker">
                    ...
                    return (
                      <div
                        ...
                        className={`relative aspect-video rounded-lg overflow-hidden border cursor-pointer hover:opacity-95 transition-all ${
                          isSelected ? 'border-blue-500 ring-2 ring-blue-500/40 opacity-100 scale-102' : 'border-slate-800 opacity-60'
                        }`}
                ...
                >
                  ...
                  <span className="text-xs text-slate-300 font-medium block">
                    Drag and drop image files here, or <strong className="text-blue-400 group-hover:underline">browse files</strong>
                  </span>
                  <p className="text-[9px] text-slate-500 mt-1 font-mono">
                    Accepts PNG, JPG, JPEG, WEBP files. Dropped files load instantly.
                  </p>
                </div>
                ...
                <div className="space-y-2 bg-slate-950/60 p-4 rounded-xl border border-slate-850">
                  <div className="flex justify-between items-center">
                    <div>
                      <h4 className="text-xs font-bold text-white uppercase tracking-wider font-mono flex items-center gap-1.5">
                        ...
                        <span>Active Listing Gallery ({photos.length} item{photos.length !== 1 ? 's' : ''})</span>
                      </h4>
                      <p className="text-[10px] text-slate-400">
                        Hold and drag items below to change their display sequence on the seeker carousel. The first photo will be the Main cover.
                      </p>
                    </div>
                    {photos.length > 0 && (
                      <button
                  ...
                  {photos.length === 0 ? (
                    <div className="text-center py-6 border border-dashed border-slate-800/80 rounded-lg">
                      <p className="text-xs text-slate-500 italic">No images in your album. Click preset cards or drop files above to populate.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3" id="active-gallery-reorder-grid">
                      {photos.map((url, idx) => {
                        ...
                        return (
                          <div
                            ...
                            id={`active-photo-${idx}`}
                          >
                            <img src={url} alt={`Catalog thumbnail ${idx}`} className="w-full h-full object-cover select-none pointer-events-none" />
              ...
              <div className="p-4 bg-slate-950/40 rounded-xl border border-slate-800 space-y-4">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    Provide real video walkthroughs. Paste any public stream link (YouTube / Vimeo) or drag over an MP4/MOV file.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-tight">Paste Stream URL link</label>
                    ...
                    <p className="text-[9px] text-slate-500 leading-normal">
                      We support rendering embedded player windows for YouTube (youtube.com, youtu.be) and Vimeo links.
                    </p>
                  </div>

                  {/* Local video uploader drop zone */}
                  <div className="space-y-2">
                    <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-tight">Or Upload Local MP4 Video Asset</label>
                    <div
                      onDragOver={(e) => e.preventDefault()}
                    ...
                    >
                      ...
                      <span className="text-[10px] text-slate-300 font-medium block">
                        Drag & Drop Video file, or <strong className="text-blue-400">Browse</strong>
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-3">Amenities List Features</label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5" id="amenities-wizard-checklist">
                  {AVAILABLE_AMENITIES.map((am) => {
                    ...
                    return (
                      ...
                      >
                        <span>{am}</span>
                        {hasAm && <Check className="w-3.5 h-3.5 stroke-3 text-blue-400" />}
                      </div>
                    );
                  })}
                </div>
          ...
          {step === 4 && (
            <div className="space-y-6 animate-fade-in" id="wizard-step-4">
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-300">Listing Status Index</label>
                  <p className="text-[10px] text-slate-400">
                    Define the lifecycle status of your property listing. This updates instantly in search grids and applicant directories.
                  </p>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3" id="listing-status-cards">
                  {[
                    { val: 'active', label: 'Active Listing', desc: 'Accepting applications', color: 'border-emerald-500/30 text-emerald-400 bg-emerald-950/25', icon: '🟢' },
                    { val: 'pending', label: 'Pending Sale', desc: 'Under review or escrow', color: 'border-amber-500/30 text-amber-400 bg-amber-950/25', icon: '⏳' },
                    { val: 'sold', label: 'Sold Out', desc: 'Ownership transferred', color: 'border-blue-500/30 text-blue-400 bg-blue-950/25', icon: '🔑' },
                    { val: 'rented', label: 'Rented / Leased', desc: 'Tenant successfully placed', color: 'border-purple-500/30 text-purple-400 bg-purple-950/25', icon: '✍️' },
                    { val: 'off-market', label: 'Off-Market', desc: 'Temporarily deactivated', color: 'border-slate-500/30 text-slate-400 bg-slate-950/25', icon: '🚫' },
                    { val: 'expired', label: 'Expired Listing', desc: 'Needs active renewal', color: 'border-red-500/30 text-red-400 bg-red-950/25', icon: '⚠️' }
                  ].map((st) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <h4 className="text-[11px] font-bold text-slate-150 leading-tight block">{st.label}</h4>
                          <span className="text-[9px] text-slate-400 leading-none truncate block mt-0.5">{st.desc}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="bg-slate-950/40 border border-slate-850 p-4 rounded-3xl space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      Elite listings receive priority placements in buyer catalog results so you convert faster than local competitors.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">Standard Package</span>
                      <span className="text-xs font-bold text-slate-300">Free</span>
                    </div>
                    <p className="text-[10px] text-slate-400 leading-snug">
                      Basic indexing. Lower view priorities. Listing expires on scheduling date unless manually monitored.
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-xs font-black text-white">$49.00</span>
                    </div>
                    <p className="text-[10px] text-slate-400 leading-snug">
                      Highlight with Golden Badge. Display first on seek results. Generates 4.5x more inquiries.
                    </p>
                  </div>
                </div>

                {isFeatured && (
                  <div className="border border-slate-800 bg-slate-950 p-4 rounded-2xl space-y-3 animate-fade-in" id="promos-checkout-node">
                    {paymentSuccess ? (
                      <div className="flex items-start gap-3 bg-emerald-950/30 border border-emerald-900/60 p-3 rounded-xl" id="checkout-completed-receipt">
                        <ShieldCheck className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />
                        <div className="min-w-0 flex-1">
                          <h5 className="text-[11px] font-bold text-white uppercase tracking-wider font-mono">Stripe Payment Confirmed</h5>
                          <p className="text-[10px] text-slate-305 mt-0.5">
                            Upgrade Active! Beautiful <strong>Gold Pro badge</strong> matches this property in search indexes.
                          </p>
                      ...
                      <div className="space-y-3" id="checkout-form-entry">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="col-span-2">
                            <label className="block text-[8px] text-slate-400 font-bold uppercase font-mono">Simulated Card String</label>
                            <input
                              type="text"
                              required={isFeatured}
                              ...
                              onChange={(e) => {
                                ...
                                setPaymentCardNumber(val.replace(/(.{4})/g, '$1 ').trim());
                              }}
                          ...
                          <div>
                            <label className="block text-[8px] text-slate-400 font-bold uppercase font-mono">Expiry MM/YY</label>
                            <input
                              type="text"
                              required={isFeatured}
                              placeholder="08/29"
                          ...
                          <div>
                            <label className="block text-[8px] text-slate-400 font-bold uppercase font-mono">CVV Secure</label>
                            <input
                              type="password"
                              required={isFeatured}
                              placeholder="•••"
                              value={paymentCardCvv}
                              onChange={(e) => setPaymentCardCvv(e.target.value.replace(/\D/g, '').substring(0,3))}
                              className="w-full bg-slate-900 border border-slate-800 rounded px-2 py-1 text-xs focus:outline-none focus:border-blue-500 text-white font-mono"
                            />
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (!paymentCardNumber) {
                              alert('Please entering your credit card details to complete payment simulation.');
                              return;
                            }
                            setIsPaying(true);
                            setTimeout(() => {
                        ...
                        >
                          {isPaying ? (
                            ...
                            <span>Pay & Activate Sovereign promotion ($49.00 USD)</span>
                          )}
                        </button>
                      </div>
                    )}
              ...
              <div className="p-4 bg-slate-950/40 rounded-3xl border border-slate-850 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      Avoid outdated listings. Keep property active on seekers’ catalog pages by enabling periodic micro-renewals.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="block text-[9px] text-slate-400 font-bold uppercase font-mono">Original Expiration Date</label>
                    <div className="relative">
                      <input
                        type="date"
                        value={expiryDate}
                  ...
                  {/* Toggle element */}
                  <div
                    onClick={() => setAutoRenewBeforeExpiry(!autoRenewBeforeExpiry)}
                  ...
                  >
                    <div className={`w-4.5 h-4.5 rounded border mt-0.5 flex items-center justify-center shrink-0 transition-colors ${
                      autoRenewBeforeExpiry ? 'bg-blue-600 border-blue-500 text-white' : 'border-slate-700'
                    }`}>
                      {autoRenewBeforeExpiry && <Check className="w-3 h-3 stroke-3" />}
                    </div>
                    <div>
                      <h4 className="text-[10px] font-bold text-slate-200 uppercase tracking-tight">Enable Auto-Renewal Option</h4>
                      <p className="text-[9px] text-slate-400 mt-0.5 leading-normal">
                        Extend for 30 more days 24 hours prior to {expiryDate || 'expiry'}. Prevents converting status to 'Expired'.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-slate-950 -mx-6 -mb-6 p-4 border-t border-slate-800 flex justify-between items-center" id="wizard-navigation">
            <button
              type="button"
              disabled={step === 1}
            ...
            {step < 4 ? (
              ...
              >
                ...
                <span>{editingProperty ? 'Apply Registry Changes' : 'Publish Property Live'}</span>
              </button>
            )}
          </div>
        </form>
